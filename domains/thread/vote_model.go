...
func vote(slugOrID string, nickname string, voiceArg int)(code int, response interface{}) {
	var voice bool
	switch voiceArg {
	case 1:
		voice = true
	...
	row := tx.QueryRow(`SELECT * FROM func_thread_vote($1, $2, $3)`, id, nickname, voice)
